...
for (feature in 1:(dim(X_RNN_full_test_maxlen5)[3]-1)) {
  ...
  scrambled_feature_mse_vector[feature] <-
    as.numeric(evaluate(complex_2_maxlen_5, scrambled_3d, 
...
The model, with very minimal tuning, Table \@ref(tab:model1-abs-acc) shows that this model can predict real GDP per capita within around $5900 USD. That's not perfect, but according to 2021 figures, that's substantially *less* than the difference between the US and Canada or the difference between Australia and the UK. It's, perhaps as expected, very directionally accurate with just the above four features.
...
```
```{r plm_var_importance_graph, cache = TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.height = 9, out.width = "100%", fig.align='center', fig.cap = "Relative Variable Importances Averaged over Linear Models"}
wdi_series %>% filter(`Series Code` %in% indicators) %>% 
  select(`Series Code`, `Indicator Name`) %>% 
  right_join(enframe(FINAL_times_plm_coef_sig), by = c("Series Code" = "name")) %>%
  mutate(indicator_clean = replace_na(`Indicator Name`, "Conflict")) %>% 
  transmute(indicator = as.factor(indicator_clean), times = value) %>%
  arrange(desc(times)) %>%
...
```{r rnn-param-comparison, echo=FALSE, eval=TRUE}
...
```{r rnn-feature-scramble, eval=TRUE, echo=FALSE}
